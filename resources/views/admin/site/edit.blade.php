...
<center><h2>友情链接修改</h2><a type="button" class="btn btn-info" href="{{url('/site/index')}}">列表</a></center>
<form class="form-horizontal" role="form" action="{{url('/site/update/'.$data->site_id)}}" method="post" enctype="multipart/form-data">
	@csrf
	<div class="form-group">
		<label for="firstname" class="col-sm-2 control-label">网站名称</label>
		<div class="col-sm-10">
			<input type="text" class="form-control" name="site_name" id="site_name" value="{{$data->site_name}}" placeholder="请输入名称">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" class="form-control" name="site_url" id="site_url" value="{{$data->site_url}}" placeholder="请输入网址">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="radio" name="site_type" value="1" @if($data->site_type==1) checked @endif>LOGO链接
			<input type="radio" name="site_type" value="2" @if($data->site_type==2) checked @endif>文字链接
		</div>
	</div>
	<div class="form-group">
		<label for="lastname" class="col-sm-2 control-label">图片LOGO</label>
		<div class="col-sm-10">
        @if($data->site_logo)<img src="{{env('UPLOAD_URL')}}{{$data->site_logo}}" width="50px">@endif<input type="file" name="site_logo">
		</div>
	</div>
	<div class="form-group">
		<label for="lastname" class="col-sm-2 control-label">网站联系人</label>
		<div class="col-sm-10">
			<input type="text" class="form-control" name="site_man" value="{{$data->site_man}}" placeholder="请输入联系人">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<textarea name="site_desc" class="form-control">{{$data->site_url}}</textarea>
		</div>
	</div>
	<div class="form-group">
		<label for="lastname" class="col-sm-2 control-label">是否显示</label>
		<div class="col-sm-10">
			<input type="radio" name="site_show" value="1" @if($data->site_show==1) checked @endif>是
			<input type="radio" name="site_show" value="2" @if($data->site_show==2) checked @endif>否
		</div>
	</div>
	<div class="form-group">
