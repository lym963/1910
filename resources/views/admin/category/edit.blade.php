...
<center><h2>分类修改</h2><a type="button" class="btn btn-info" href="{{url('/category/index')}}">列表</a></center>
<!-- @if ($errors->any()) 
...
@endif -->
<form class="form-horizontal" role="form" action="{{url('/category/update/'.$data->cate_id)}}" method="post">
	@csrf
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<select name="pid">
				...
				@foreach($info as $v)
				<option value="{{$v->cate_id}}" @if($data->pid==$v->cate_id) selected @endif >{!!str_repeat("&nbsp;",$v["level"]*5)!!}{{$v->cate_name}}</option>
				@endforeach
			</select>
		</div>
	</div>
